...
function buildRecipeJson(fields: Partial<Recipe> & { name: string }): Recipe {
  const now = isoNow();
  return {
    id: String(Date.now()),
    name: fields.name,
...
function formatRecipeSummary(folderName: string, recipe: Recipe): string {
...
function formatRecipeDetail(folderName: string, recipe: Recipe): string {
  const lines: string[] = [];
  ...
  if (recipe.recipeInstructions.length > 0) {
    ...
    recipe.recipeInstructions.forEach((step, idx) => lines.push(`${idx + 1}. ${step}`));
    lines.push('');
  }
...
async function readAllRecipes(): Promise<{ folderName: string; recipe: Recipe }[]> {
  ...
  const results = await Promise.allSettled(
    dirs.map(async (dir: { basename: string }) => {
      const json = await client.getFileContents(`${RECIPES_ROOT}/${dir.basename}/recipe.json`, {
        format: 'text',
      });
      return {
...
export const listRecipesTool = {
  ...
  inputSchema: z.object({
    ...
    category: z.string().optional().describe('Filter by recipe category'),
    keyword: z.string().optional().describe('Filter by keyword'),
  }),
  handler: async (args: { search?: string; category?: string; keyword?: string }) => {
    try {
      let entries = await readAllRecipes();

      if (args.search) {
        const s = args.search.toLowerCase();
        entries = entries.filter((e) => e.recipe.name.toLowerCase().includes(s));
      }
      if (args.category) {
        const c = args.category.toLowerCase();
        entries = entries.filter((e) => e.recipe.recipeCategory.toLowerCase() === c);
      }
      if (args.keyword) {
          ...
          content: [
            {
              type: 'text' as const,
              text:
                args.search || args.category || args.keyword
      ...
      const formatted = entries.map((e) => formatRecipeSummary(e.folderName, e.recipe)).join('\n');
...
export const listRecipeCategoriesTool = {
  ...
  handler: async () => {
    try {
      ...
      const categories = [
        ...new Set(entries.map((e) => e.recipe.recipeCategory).filter((c) => c !== '')),
      ].sort();

      if (categories.length === 0) {
        return {
          content: [{ type: 'text' as const, text: 'No recipe categories found.' }],
        };
      }
...
export const getRecipeTool = {
  ...
  handler: async (args: { folderName: string }) => {
    try {
      const client = getWebDAVClient();
      const json = await client.getFileContents(`${RECIPES_ROOT}/${args.folderName}/recipe.json`, {
        format: 'text',
      });
...
const nutritionSchema = z
  .object({
    ...
    carbohydrateContent: z.string().optional().describe("Carbohydrates (e.g., '2 g')"),
    proteinContent: z.string().optional().describe("Protein (e.g., '3 g')"),
...
export const createRecipeTool = {
  ...
  inputSchema: z.object({
    ...
    prepTime: z.string().optional().describe('Preparation time in ISO 8601 duration (e.g., PT30M)'),
    cookTime: z.string().optional().describe('Cooking time in ISO 8601 duration (e.g., PT1H)'),
    totalTime: z.string().optional().describe('Total time in ISO 8601 duration'),
    recipeCategory: z.string().optional().describe("Category (e.g., 'Easy', 'Dessert')"),
    keywords: z.string().optional().describe("Comma-separated keywords (e.g., 'pasta,italian')"),
    ...
    recipeInstructions: z.array(z.string()).optional().describe('Step-by-step instructions'),
...
export const updateRecipeTool = {
  ...
  inputSchema: z.object({
    ...
    totalTime: z.string().optional().describe('New total time (ISO 8601 duration)'),
    ...
    recipeIngredient: z.array(z.string()).optional().describe('New ingredients list'),
    recipeInstructions: z.array(z.string()).optional().describe('New instructions list'),
    nutrition: nutritionSchema,
  }),
  handler: async (args: {
    folderName: string;
    ...
    try {
      ...
      await client.putFileContents(recipePath, JSON.stringify(recipe, null, 2), {
        overwrite: true,
      });
...
export const deleteRecipeTool = {
  ...
  inputSchema: z.object({
    folderName: z.string().describe('The recipe folder name to delete'),
  }),
  handler: async (args: { folderName: string }) => {
    try {
      const client = getWebDAVClient();
