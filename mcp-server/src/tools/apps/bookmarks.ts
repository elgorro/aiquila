...
function formatFolderTree(folder: BookmarkFolder, indent = 0): string {
  ...
  if (folder.children) {
    for (const child of folder.children) {
      if ("url" in child) {
        lines.push(`${prefix}  - ${(child as Bookmark).title || "(untitled)"} — ${(child as Bookmark).url} (ID: ${child.id})`);
      } else {
...
export const listBookmarksTool = {
  ...
  inputSchema: z.object({
    ...
    tags: z
      .array(z.string())
      .optional()
      .describe("Filter by tags (e.g. ['tech', 'news'])"),
    folder: z
      .number()
      .optional()
      .describe("Filter by folder ID (-1 for root)"),
    page: z
      .number()
      .optional()
    ...
    limit: z
      .number()
      .optional()
      .describe("Max bookmarks to return (default 10)"),
    sortby: z
    ...
    untagged: z
      .boolean()
      .optional()
      .describe("Only show bookmarks without tags"),
    unavailable: z
      .boolean()
      .optional()
      .describe("Only show unavailable bookmarks"),
  }),
  handler: async (args: {
    search?: string;
    tags?: string[];
    ...
    try {
      ...
      const result = await fetchBookmarksAPI<BookmarksListResponse>(
        "/bookmark",
        { queryParams }
      );
...
export const getBookmarkTool = {
  ...
  handler: async (args: { id: number }) => {
    try {
      const result = await fetchBookmarksAPI<BookmarkItemResponse>(
        `/bookmark/${args.id}`
      );
...
export const createBookmarkTool = {
  ...
  inputSchema: z.object({
    ...
    tags: z
      .array(z.string())
      .optional()
      .describe("Tags to assign (e.g. ['tech', 'reference'])"),
    folders: z
      .array(z.number())
      .optional()
      .describe("Folder IDs to place the bookmark in"),
  }),
  handler: async (args: {
    url: string;
    title?: string;
...
export const updateBookmarkTool = {
  ...
  inputSchema: z.object({
    ...
    folders: z
      .array(z.number())
      .optional()
      .describe("New folder IDs (replaces existing folders)"),
  }),
  handler: async (args: {
    id: number;
    url?: string;
...
export const deleteBookmarkTool = {
  ...
  handler: async (args: { id: number }) => {
    try {
      await fetchBookmarksAPI<StatusResponse>(
        `/bookmark/${args.id}`,
        { method: "DELETE" }
      );
...
export const listBookmarkFoldersTool = {
  ...
  inputSchema: z.object({
    ...
    layers: z
      .number()
      .optional()
      .describe("Maximum depth of folder levels to return"),
  }),
  handler: async (args: { root?: number; layers?: number }) => {
    try {
      ...
      const result = await fetchBookmarksAPI<FolderListResponse>(
        "/folder",
        { queryParams }
      );
      ...
      if (result.data.length === 0) {
        return {
          content: [
            { type: "text" as const, text: "No bookmark folders found." },
          ],
        };
      }

      const formatted = result.data
        .map((f) => formatFolderTree(f))
        .join("\n");
  ...
  handler: async (args: { id: number; layers?: number }) => {
    try {
      ...
      if (result.data.length === 0) {
        return {
          content: [
            { type: "text" as const, text: "Folder is empty." },
          ],
        };
      }
...
export const createBookmarkFolderTool = {
  ...
  inputSchema: z.object({
    ...
    parent_folder: z
      .number()
      .optional()
      .describe("Parent folder ID (-1 for root, default)"),
  }),
  handler: async (args: { title: string; parent_folder?: number }) => {
    try {
      ...
      const result = await fetchBookmarksAPI<FolderItemResponse>(
        "/folder",
        { method: "POST", body }
      );
...
export const updateBookmarkFolderTool = {
  ...
  inputSchema: z.object({
    ...
    parent_folder: z
      .number()
      .optional()
      .describe("New parent folder ID (-1 for root)"),
  }),
  handler: async (args: {
    id: number;
    title?: string;
    ...
    try {
      ...
      await fetchBookmarksAPI<FolderItemResponse>(
        `/folder/${args.id}`,
        { method: "PUT", body }
      );
  ...
  handler: async () => {
    try {
      ...
      if (result.data.length === 0) {
        return {
          content: [
            { type: "text" as const, text: "No bookmark tags found." },
          ],
        };
      }

      const formatted = result.data.sort().map((t) => `- ${t}`).join("\n");
...
export const renameBookmarkTagTool = {
  ...
  handler: async (args: { old_name: string; new_name: string }) => {
    try {
      await fetchBookmarksAPI<StatusResponse>(
        `/tag/${encodeURIComponent(args.old_name)}`,
        { method: "PUT", body: { name: args.new_name } }
      );
...
export const deleteBookmarkTagTool = {
  ...
  handler: async (args: { name: string }) => {
    try {
      await fetchBookmarksAPI<StatusResponse>(
        `/tag/${encodeURIComponent(args.name)}`,
        { method: "DELETE" }
      );
