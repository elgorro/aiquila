...
# Returns 0 (true) if the tool is available, 1 if the app is missing or errored.
# Used as a pre-check: call the list tool first, then gate subsequent CRUD tests.
check_app_available() {
  python3 -c "
import json, sys
raw = open('$TMPFILE').read().strip()
...
sep "MCP 54 — talk_list_conversations (pre-check)"
...
sep "MCP 58 — list_notes (pre-check)"
...
sep "MCP 64 — list_tasks (pre-check for Tasks app)"
